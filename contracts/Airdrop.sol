...
contract Airdrop is Ownable, Pausable{
    ...
    IERC20 public token;
    uint256 public totalDistributedAirdropTokens;
    ...
    function bulkTransfer(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            require(
                amounts[i] > 0,
                "Invalid amount: should not be zero"
            );

            token.safeTransfer(recipients[i], (amounts[i] * 10 ** 18));
            totalDistributedAirdropTokens += amounts[i];
        }
    ...
    function checkAllowance(address _address)public view returns(uint256){
